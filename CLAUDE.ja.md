...
- **モデルロード**（`src/lib/model.ts`）: ベースモデルは `rinna/japanese-gpt2-xsmall`（MITライセンス、GPT2アーキテクチャ、6層・512隠れ層、sentencepiece/T5Tokenizer）。そのONNX変換版 `saldra/rinna-japanese-gpt2-xsmall-onnx`（同じくMITライセンス）をローカルにコピーし、`public/models/rinna-japanese-gpt2-xsmall/` にコミット済み（量子化ONNXファイルはtransformers.js標準命名に合わせて `onnx/model_quantized.onnx` にリネーム）。**実行時にHugging Face Hubからは読み込まない**（`env.allowRemoteModels = false`。`env.localModelPath` はViteの `import.meta.env.BASE_URL` から組み立てているので `/GPT2Viewer/` のPagesサブパス配下でも正しく解決する）。
  - この量子化版はデフォルトだと **onnxruntime-web(wasm)ではロードに失敗する**（`Missing required scale: ...wte.weight_merged_0_scale for node: ...DequantizeLinear`）。onnxruntime-node（Node.js）では問題なくロードできてしまうため、Nodeだけのテストでは気づきにくい落とし穴だった。`quantize_config.json`を直接確認したところ`weight_type: QInt8`（8bit）であり、当初ここに書いていた「int4 blockwise/MatMulNBits」という理解は誤りだったので訂正。原因は（ソースまで完全に裏取りはしていないが）おそらく: このファイルはQDQ形式（`DequantizeLinear`を明示的に持つ形式）で量子化されており、GPT2は`wte.weight`とLM headが同一テンソル（tied weights）であり、ONNX Runtimeの重複排除最適化がこのテンソルをリネームする一方、QDQ→`MatMulNBits`融合パス（`TransposeDQWeightsForMatMulNBits`、`'extended'`/`'all'`最適化レベルで有効。この融合パスはint4専用ではなく、QDQ形式のint8にも適用される）がリネーム後の名前でスケール初期化子を探しに行って見つからない、という食い違い。**対処法**: `from_pretrained` に `session_options: { graphOptimizationLevel: 'basic' }` を渡すことで、この融合パス自体をスキップすれば同じ40MBファイルがそのままロードできる。このモデルを読み込む際は必ず必要な設定なので外さないこと。
  - 別案として自前変換も試した: conda環境 `ai-tools`（Python 3.10、プロジェクト専用ではなくAI/ML作業全般向け — 新規作成せず再利用すること）に `optimum-onnx` + `onnx` + `onnxruntime` + `torch`。変換: `optimum-cli export onnx --model rinna/japanese-gpt2-xsmall --task text-generation-with-past <dir>`（transformers.js標準命名と一致する単一の `model.onnx` が出力される）。量子化: `onnxruntime.quantization.quantize_dynamic(..., weight_type=QuantType.QUInt8)`（標準dynamic int8、QOperator形式 — `MatMulNBits`を生成しないため上記バグを踏まない）。結果: 215MB→約54MB。saldra版（40MB、こちらもint8・QDQ形式でint4ではない）と比較（プロンプトごとの次トークン候補トップ10の一致度6〜8/10、greedy/サンプリングいずれでも明確な品質差なし）した結果、より小さいsaldra版＋`graphOptimizationLevel`回避策を採用し、自前変換は見送った（独自の変換パイプラインを維持するコストに見合わないため）。※両方ともint8なので、この比較からint4化の可能性については何もわからない点に注意（実際、`rinna/japanese-gpt2-small`のint4量子化を試みた際は現行ツール群の実力不足に直面した。詳細は`training-data/`配下の各READMEを参照）。
...
- **UI構成**: `src/App.tsx`が全状態（`scene`、選択単語/文体、生成の進行状況）とモデルキャッシュ（`modelCacheRef`、`Map<modelId, {model, tokenizer}>`）を持つ。モデルはマウント時にプリロードせず、「作文する」を押したタイミング（`ensureModel`）で選択中のスタイルのものだけを遅延ロードする — 使わないモデルのメモリコストを一切払わないようにするため。「もう一度体験する」（`handleRestart`）ではキャッシュ済みの全モデルのONNXセッションを`model.dispose()`で明示的に解放してからキャッシュをクリアするので、次にスタイルを選ぶと必ず再ロードになる（JSの参照を捨てるだけではWASM側の線形メモリが確実に解放される保証がないため）。再体験のたびに数秒の再ロードが発生するトレードオフはあるが、「常に最大1モデル分しかメモリに乗らない」ことを優先した設計 — この展示はキオスク的な使われ方（同じページのまま多人数が入れ替わり利用、リロードなし）を想定しており、スタイルをまたいでキャッシュが際限なく積み上がるとiPhone/モバイルSafariの厳しいメモリ制限では実害が出るため。各シーンは`src/components/`配下の表示用コンポーネント: `WordSelectStage.tsx`, `GenerationStage.tsx`, `ResultStage.tsx`。シーン2の1トークンごとの見せ方はあえて2段階（`App.tsx`冒頭の`CANDIDATE_DISPLAY_MS` / `CHOSEN_HOLD_MS`定数）— まず候補と確率バーを何もハイライトせずに表示し、その後サンプリングされたトークンをハイライトして少し止め、それから文章に追加する、という流れにすることで「確率→選択」の瞬間が一瞬で流れずに見えるようにしている。生成のペース・長さを変更する依頼が来たら、まずこの2定数と`MAX_NEW_TOKENS`を見ること。
