...
- **シーン1**（`WordSelectStage`）: タイトル「さくぶんAI」→ 単語を3つ選択 → 文体（小説/名言/詩、それぞれ専用にファインチューニングしたモデルに接続済み — Step 10参照）を選択 → 「作文する」
...
- [x] 名言スタイル用コーパス収集: Wikiquote日本語版（CC BY-SA）から、著作権消滅済み（没後70年超）の歴史上の人物15名（松尾芭蕉・夏目漱石・徳川家康・坂本龍馬・福澤諭吉など）の発言をMediaWiki API経由で取得・クリーニング。153件・約5,500文字。既存の「名言集」の丸ごと利用は編集著作物の権利に触れる可能性があるため、個々の発言を人物ページから収集する方式にした。`training-data/meigen/corpus.json`/`corpus.txt`
...
- [x] 名言スタイルのファインチューニング（童謡と同じ80epochスケジュール方式、epoch45〜80を比較して**epoch55を採用**）→ ONNX変換・量子化（38MB）→ `public/models/rinna-japanese-gpt2-xsmall-meigen/`へ配置、`src/lib/model.ts`に`MEIGEN_MODEL`として追加、`src/App.tsx`の「名言」スタイルに接続。詳細・わかったことは`training-data/meigen/README.md`参照
...
- [x] `src/App.tsx`の`STYLE_OPTIONS`を実際の3モデルに接続完了。Plan.mdの「3つの文体で生成」というMVP要件を満たした
- [x] Plan.mdのMVP範囲外だが、ファインチューニング前後の違いを直接比較できるよう「オリジナル」（`BASE_MODEL`、未ファインチューニング）を4つ目の文体選択肢として追加
...
- **超短文コーパス（名言）特有の罠**: 発言が短くEOSで即終了する学習データが大半だと、素の`model.generate()`（既定でEOS到達即停止）で試すと「文体変化なし」に誤診しかねない。実アプリは`allowEos: false`でEOSを候補から除外するため、評価もEOS除外込みで行わないと正しく判断できない
